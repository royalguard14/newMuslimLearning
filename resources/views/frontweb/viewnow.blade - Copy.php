...
<div class="message-container">
  <?php $chatcount = 1; ?>
  @foreach($videoChats as $chat)
  <?php $chatcount++ ?>
  <div class="message anim" style="--delay: .{{$chatcount}}s">
    <div class="author-img__wrapper video-author video-p">
     <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check">
      <path d="M20 6L9 17l-5-5" />
    </svg>
  ...
  <div class="msg-wrapper">
    ...
    ->select('id', 'first_name', 'middle_name', 'last_name','online')
    ->first();
    ?>
    <div class="msg__name video-p-name {{ $user->online ? '' : 'offline' }}">
      @if($user)
      <?php
    ...
    <div class="msg__content video-p-sub"> {{ $chat->message }}</div>
  </div>
</div>
@endforeach
</div>
...
<div class="chat-vid__button anim" style="--delay: .6s" onclick="window.location.href ='{{route('web.library')}}'">See All videos ({{Videos::count()}})</div>
</div><!-- end -->
</div>
</div>
@endsection